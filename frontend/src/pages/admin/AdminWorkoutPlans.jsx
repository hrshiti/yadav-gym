...
const AdminWorkoutPlans = () => {
  ...
  const [formData, setFormData] = useState({
    planName: '',
    duration: '',
    exercises: [{ id: 1, name: '', sets: 3, reps: 10, rest: '30s', notes: '' }],
  });
    ...
    if (viewMode === 'add') {
      const newPlan = {
        id: activeTab === 'members' 
          ? `MP${String(memberPlans.length + 1).padStart(3, '0')}`
          : `TP${String(trainerPlans.length + 1).padStart(3, '0')}`,
  ...
  // Add exercise
  const addExercise = () => {
    const newId = Math.max(...formData.exercises.map(e => e.id), 0) + 1;
    setFormData({
      ...formData,
      exercises: [...formData.exercises, { id: newId, name: '', sets: 3, reps: 10, rest: '30s', notes: '' }],
    });
  };
        ...
        {(viewMode === 'add' || viewMode === 'edit') && (
          ...
          >
            <h3 className="text-base sm:text-lg font-heading font-bold text-text-dark mb-4">
              {viewMode === 'add' ? 'Add New Workout Plan' : 'Edit Workout Plan'}
            </h3>
            <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
              ...
              <div>
                <div className="flex items-center justify-between mb-3">
                  <label className="block text-sm font-medium text-text-dark">Exercises *</label>
                  <motion.button
                    type="button"
                    onClick={addExercise}
                ...
                <div className="space-y-3">
                  {formData.exercises.map((exercise, index) => (
                    <div key={exercise.id} className="p-3 bg-gray-50 rounded-lg border border-gray-200">
                      <div className="flex items-center justify-between mb-2">
                        <span className="text-xs font-medium text-text-dark">Exercise {index + 1}</span>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        <div>
                          <label className="block text-xs font-medium text-text-dark mb-1">Exercise Name *</label>
                          <input
                            type="text"
                            required
                            value={exercise.name}
                        ...
                        <div>
                          <label className="block text-xs font-medium text-text-dark mb-1">Sets *</label>
                          <input
                            type="number"
                            required
                            min="1"
                        ...
                        <div>
                          <label className="block text-xs font-medium text-text-dark mb-1">Reps/Time *</label>
                          <input
                            type="text"
                            required
                            value={exercise.reps}
                        ...
                        <div>
                          <label className="block text-xs font-medium text-text-dark mb-1">Rest Time</label>
                          <input
                            type="text"
                            value={exercise.rest}
          ...
          >
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 mb-3 sm:mb-4">
              <h3 className="text-base sm:text-lg font-heading font-bold text-text-dark">
                {activeTab === 'members' ? 'Members' : 'Trainers'} Workout Plans
              </h3>
              {selectedDate && (
            ...
            {plans.length === 0 ? (
              <p className="text-text-light text-center py-8">No workout plans found</p>
            ) : (
              <div className="space-y-3 sm:space-y-4">
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="mt-3 pt-3 border-t border-gray-200">
                        <h6 className="text-xs sm:text-sm font-semibold text-text-dark mb-2">Exercises:</h6>
                        <div className="space-y-2">
                          {plan.exercises.map((exercise, exIndex) => (
                            <div key={exIndex} className="flex items-center justify-between text-xs sm:text-sm bg-white p-2 rounded border border-gray-100">
                              <div className="flex-1">
                                <span className="font-medium text-text-dark">{exercise.name}</span>
                                <span className="text-text-light ml-2">
                                  {exercise.sets} sets × {exercise.reps} {exercise.rest && `| Rest: ${exercise.rest}`}
                                </span>
                              </div>
                              {exercise.notes && (
                                <span className="text-text-light text-xs ml-2">{exercise.notes}</span>
                              )}
                            </div>
                          ))}
                        </div>
